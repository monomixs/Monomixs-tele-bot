...
async def get_command_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Check for conflicts within the specific scope (group or user)
    if chat_type in ["group", "supergroup"]:
...
def main() -> None:
    ...
    # Add the single, smart handler for all custom commands. It has a lower priority
    # so the CommandHandlers for premade commands are checked first.
